...
1. `M3-W2-ARCHIVETEX-06-R3` 与 `M3-W2-ARCHIVEINTEGRATE-07-R2` 已在 `fef30d0` 上由 A2 独立 PASS：archive 默认并行压力 20/20，全仓串行 175 PASS、5 个 ignored 专跑 PASS；不再是 G3 缺口。
2. 原版实测 `CurView.Excentry` 的来源是隔离前 GLWorkArea `1280×721`，不是最终截图 `1280×720`；Rust 调试相机仍按 `1280/720`，须改用真实比例后重新生成共同相机六槽。
3. Rust 当前只有 12 张 BMP 和 `capture.json`；原版调试目录混有多轮候选图，且 `CopyFromScreen` 曾被桌面聊天窗口遮挡。正式门禁仍缺干净 SHA 目录、PNG、完整程序/样本/MPQ/脚本 manifest、open/reopen 各两次和正常关闭证据。
4. 调试采集的 Ember 红通道 MAE 与 Nether 可见性差异只能作为返工线索；共同相机和无污染正式图冻结前，不修改着色器或 ScenePacket 语义。PRE2/RIBB 已正式排除为 G3 效果缺口，`N47` 路由 M5 `TOOLS-02`。
...
| M3-W2-ARCHIVETEX-06-R3 | A7 / 规范工作树 | 仅 `src/texture/archive.rs` 测试辅助与测试 | 不得修改生产档案读取语义、Cargo、模块入口或 app | `e76f68052be128d9dd0a6fef58a63364f931bcb9` | 已完成：`fef30d0b487b5dbdbcb59e37ae7ad32e7c4a1ff2`，A2 独立 PASS；串行/默认并行与 20 轮压力无 flake，生产逻辑零改动 |
| M3-W2-ARCHIVEINTEGRATE-07-R2 | A0 / 规范工作树 | 仅 `src/app/app.rs` 集成测试与 G3 ignored 采集测试 | 不得修改 Model/Pose/ScenePacket/renderer/texture 生产语义；不得与其他任务并行写 app.rs | `e76f68052be128d9dd0a6fef58a63364f931bcb9` | 已完成：`fef30d0b487b5dbdbcb59e37ae7ad32e7c4a1ff2`，A2 独立 PASS；非法生产配置旧 Scene 保留，app 集成串行/默认 12 PASS / 2 ignored |
| M3-W2-CAPTURE-08-R1 | A0 / 规范工作树 + 隔离验证环境 | `Cargo.toml`、`Cargo.lock`、`src/app/app.rs` ignored 采集测试；证据仅写 `%TEMP%` | 不得修改 Model/Pose/ScenePacket/renderer/texture/WGSL 生产语义；原版目录只读 | `fef30d0b487b5dbdbcb59e37ae7ad32e7c4a1ff2` | 执行中：Rust 改用原版 `1280/721` 相机比例并输出 PNG/manifest/open+reopen 双重复；原版在干净 SHA 目录记录程序/样本/MPQ/脚本身份并防屏幕遮挡 |
| M3-W2-VIEWVERIFY-05-R1 | A1+A2 / 隔离验证环境 | 仓库只读；证据写 `%TEMP%/mdlvis-g3-viewverify/<新 SHA>/` | CAPTURE-08-R1 | 待新集成 SHA | 阻塞：`fef30d0` 的 archive/app 返工已通过，但只有 Rust BMP 调试六槽与混合原版候选目录；等待正式 PNG/manifest/open+reopen 双重复后执行 v1 MAE |
...
除 A10 的文档任务外，所有工作包默认禁止修改正式 `docs/`；除 A0 的集成任务外，默认禁止修改共享热点；所有工作包均禁止修改两个原版参考目录。注册表共 28 个交付工作包和 7 个阶段门禁。
...
| M3-W2-CAPTURE-08-R1 | A0 / A2 | 用真实原版相机比例补齐双端正式 PNG、manifest 与 open/reopen 双重复 | ARCHIVETEX-06-R3、ARCHIVEINTEGRATE-07-R2 | `Cargo.toml`、`Cargo.lock`、`src/app/app.rs` ignored 测试；临时原版自动化与证据目录 | 双端槽位一一对应、身份字段完整、无桌面遮挡、重复稳定，可执行 v1 MAE |
...
| 2026-08-14 | fef30d0 两项返工独立 PASS 并登记正式采集闭环 | `ARCHIVETEX-06-R3`、`ARCHIVEINTEGRATE-07-R2`、`CAPTURE-08-R1`、`VIEWVERIFY-05-R1` | A2 证明 archive 20/20、全仓 175 PASS 与 5 ignored 专跑通过；审查确认 G3 只剩 BMP/manifest/open-reopen、原版屏幕遮挡和真实 `1280/721` 相机比例 | 用户要求按实际内容继续推进 |
