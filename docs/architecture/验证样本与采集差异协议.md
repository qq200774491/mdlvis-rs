...
| --- | --- | --- | --- | --- |
| 近似静态网格 | `Nether Blast/Nether Blast I.mdx` | 1 个几何体、1 根骨骼、无父节点、无 KGRT；附带 KMTA 与 Bezier 缩放 | 是（Hive，署名 Vinz） | Birth 中段帧、相机尚未钉死 |
| 层级动画 | `Arthas.mdx` | 13 个序列、2 个全局序列、深度 8、KGRT 以 Hermite 为主、最多 4 骨混合 | 否，仅本地对照 | 在修好旋转 SLERP 之前不得冻结姿态金样 |
| 复杂材质 | `Ember Forge  Ember Knight/Ember Forge_opt2.mdx` | 24 层、KMTA、TXAN、GEOA、RID 1+2 | 是（Hive，署名 Vinz / Moy） | 选一个不依赖未实现粒子的 Stand 观察序列 |
| 负例 | `QDMR.mdx` | 魔数 `EMHM`，不是 MDX | 否 | 期望：明确失败，不 panic |

候补：`Ember Knight_opt2.mdx`（本地 `EmberKnight.blp`）。不要用 `Fire_Stream.mdx` 当静态网格（1 个顶点）。仓库内仍缺少授权清晰、无发射器的纯静态网格；阶段 0 出口前应补裁剪夹具或另选授权样本。
...
| 结构转储 | 无官方导出 | `verification::dump_structure` 输出 JSON；G1 使用完整当前 Model JSON 做格式语义门禁 | 姿态 dump 仍缺 |
| 逐帧姿态 | 无官方导出 | `AnimationSystem::update` 可算骨骼/辅助节点 | 无快照测试；公告牌 TODO；SLERP 未接通前不冻 Arthas |
| 截图 | 仅 GUI；正式路径为 UI 自动化 | 无稳定离屏截图 API | 需 `COPY_SRC` 与无窗口捕获 |
| 自动化 | 无命令行；格式互开使用受控 UI 自动化 | `cargo test --bin mdlvis-rs verification` 与格式模块测试可跑结构/往返门禁 | 姿态与离屏截图仍待 M2/M3 落地 |
