...
未单独标注的菜单项，默认按其所在功能域进入后续里程碑：文件/查看进入 M3，对象/动画编辑进入 M4，几何/UV/优化进入 M5。例外是 `N47`：源码证明它只切换 PRE2 节点辅助标记，不是粒子效果渲染，随其他辅助显示与面板状态进入 M5 `TOOLS-02`。
...
| 视图 / 对象 / 粒子源 | `N47` | 无 | `N47Click` | `frmmain.dfm:4784`；`mdlDraw.pas:915-937` | 切换 PRE2 节点绿色四面体辅助标记；不模拟发射效果 | 查看辅助 | 工具 | Model 已解析 PRE2；辅助叠加随 M5 `TOOLS-02` |
...
| 几何体 | `TGeoset` | 非节点 | 分离时产生 | 顶点、法线、多 UV、面、矩阵组、材质 | `mdlwork.pas:82-112` | Model 与 MDL/MDX 读写已覆盖有序多 UV 集；renderer 暂只机械消费 UV0，完整 `CoordId` 渲染待 M3 |
...
原版 Full View 的主体路径 `AMake3DListFill` 只绘制 Geoset 材质层；`DrawObjPart -> DrawObjects` 负责骨骼、附着点、PRE2 等辅助叠加。`VisParticles` 分支只为 PRE2 调用 `DrawTetrahedron`，未发现 PRE2 粒子模拟/效果绘制；RIBB 也没有进入 `mdlDraw` / `guic` 的效果绘制或模拟。因此格式层仍完整保留 PREM/PRE2/RIBB 数据，但 G3 截图不以“实现原版不存在的粒子/飘带效果”为出口条件。
...
| 几何体 / 几何体动画 | `GEOS` / `GEOA` | 读 | 写 | `MDXReadGeosets` 4096–4197；`MDXReadGeosetAnims` 4200–4230 | 读写 | `UVAS` 与全部有序 `UVBS`、重复 MDL `TVertices`、矩阵组和 GEOA 语义均保留；非法 UV 长度在覆盖目标前拒写 |
...
| 域 | 已观察事实 | 证据 |
| --- | --- | --- |
| 桌面 | 可打开模型；有纹理、显示、颜色、模型信息、几何体、材质和动画面板；界面使用 Fluent `zh-CN` / `en-US`，默认 `zh-CN` | `src/ui.rs`、`src/i18n.rs`、`locales/` |
| 格式 | MDX/MDL VERS 800 读写；当前 Model 已建模对象、轨道与多 UV 语义往返；MDX unknown/MDVI 分舱保存 | `src/parser/`、`src/format/mdl/`、G1 固定 SHA `ffae8171`、多 UV 门禁 `de47cac9` |
| 动画 | `Pose/FrameContext` 已覆盖 SLERP、Hermite/Bezier、序列/全局双时钟、层级、公告牌、可见性和 CPU 蒙皮输入；G2 固定 SHA 已独立通过 | `src/animation/`、G2 固定 SHA `1b8221f8` |
| 材质与渲染 | KMTA/KMTF、TXAN、GEOA 进入运行时姿态和 `ScenePacket`；七种过滤模式、关键深度/剔除/排序状态、BLP 与 MPQ 纹理源已有专用测试 | `src/material/`、`src/scene/`、`src/renderer/`、`src/texture/` |
| 编辑 | 格式层已有保存 API；尚无命令历史和对象树 | `src/parser/write.rs`、`src/format/mdl/writer.rs`；`src/` 无 `editor` 模块 |
| 测试 | G1/G2 已通过；M3 基线 `e76f680` 串行全仓 174 PASS、5 个 ignored 专跑 PASS，G3 仍等待共同相机截图满足 v1 MAE | G1/G2 门禁记录、当前推进计划 |
...
| 2026-08-14 | 补齐多 UV 格式契约 | `Geoset` 以有序 UV 集为唯一真相；MDX/MDL 双格式原版打开保存重开与固定 SHA 回读通过。 |
| 2026-08-14 | 修正 PRE2/RIBB 可观察范围 | `N47` 仅为 PRE2 四面体辅助叠加并路由 M5 `TOOLS-02`；原版 Full View 无粒子/飘带效果模拟，G3 不以虚构效果为门禁。 |
